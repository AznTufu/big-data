...
def aggregate_client_behavior(fact_table: pd.DataFrame) -> pd.DataFrame:
    ...
    client_behavior["purchase_frequency"] = (
        ...
        (client_behavior["customer_lifetime_days"] + 1)
    )
...
def gold_aggregation_flow(clients_file: str = "clients.parquet", purchases_file: str = "purchases.parquet") -> Dict:
    """
    ...
    df_purchases = load_data_from_silver(purchases_file)
    df_clients = load_data_from_silver(clients_file)
